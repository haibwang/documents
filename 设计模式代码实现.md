...
/**! 定义抽象类，针对该类编程，不关心具体实现*/
public  abstract class Duck {
    
	Ifly flyBehaviour;      /**< 声明为接口，不关心接口的实现，通过动态加载接口的实现*/
	Iquack quackBehaviour;
	
	void performFly(){
		flyBehaviour.flying();  /**< 接口的方法，针对接口编程，只知道改接口是如何实现的，不关心*/
	};
...
//接口的实现
public class flyMallard implements Ifly {
...
public class MallardDuck extends Duck {
	...
	public MallardDuck(){
		flyBehaviour = new flyMallard();
		quackBehaviour = new quackMallard();/**< 动态加载 */
	}
	@Override
	void display() {
		// TODO Auto-generated method stub
		System.out.print("i have a white plumage!\n");
	}
	
}
针对不同的鸭子可以有不同的实现，只需要在创建鸭子的时候动态加载该实现即可，这样，来了一只回翻跟斗的鸭子时，是创建一个翻跟斗的实现类，而并不影响之前创建好了的鸭子。

public class main {
	   public static void main(String[] args) {
		// TODO Auto-generated method stub
        Duck mallard = new MallardDuck(); /**< 继承超类，实现接口的动态加载*/
        mallard.display();
        mallard.performFly();
...
main()
{
    ...
    wooden_duck.init(&wooden_duck);/**<调用构造方法,构造方法总是能看见自己的属性，所以把自己传进去*/
